...
class W300LP(data.Dataset):
    ...
    def generateSampleFace(self, idx):
        ...
        #tpts = pts.clone() #instead of clone copy() is used
        tpts = pts.copy()
        tpts = torch.FloatTensor(tpts.astype(np.float64)) # can't assign a numpy.float32 to a torch.FloatTensor. we can assign float64.
